...
- [ ] **[S] `--stream` flag to force token streaming on non-TTY stdout** — the CLI only streams when stdout is a TTY (`cmd/root.go` `stdoutTTY`), so piped consumers (the Raycast extension, `foo | translate`) get the whole result at once. A `--stream` flag setting `stream := stdoutTTY || flagStream` would unblock progressive/streamed output in the Raycast `Translate` view (spawn + append stdout to `Detail.markdown`). See [docs/raycast-extension.md](docs/raycast-extension.md) and [backlog/raycast-extension.md](backlog/raycast-extension.md).
