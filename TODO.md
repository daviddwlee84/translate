...
- [ ] **[?/S] Frequency-aware "did you mean" suggestion ranking** — the dict engine ranks fuzzy suggestions by edit distance only, so the common correction isn't first (`recieve` → `receive` is 6th). Re-rank by (distance, then ECDICT `frq`/`bnc`) in `internal/engine` so all frontends (CLI/Raycast/MCP/serve) benefit; optionally expose `SuggestDistance` in `--json` for a Google-style auto-correct. → [research](backlog/suggestion-ranking.md)
