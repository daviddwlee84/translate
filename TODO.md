...
- [ ] **[?/L] Local HTTP API server (`translate serve`) + OpenAPI/Swagger** — a persistent loopback service (`POST /v1/translate`, `/v1/define`, `GET /v1/history`, SSE stream) would give the Raycast extension (and other clients: Alfred, browser, editors) cleaner streaming, multi-client reuse, and no launchd-PATH resolution. Trade-off: daemon lifecycle + loopback security + concurrent history writes; latency win is small (LLM-dominated). Reuses `internal/engine`. → [research](backlog/api-server.md)
